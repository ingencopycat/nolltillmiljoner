...
(() => {
  const keys = { theses: 'investment-research-theses-v1', outcomes: 'ntm-research-outcomes-v1',
    scenarios: 'investment-scenarios-v1', theme: 'investment-theme' };
  ...
  function counts(data) {
    return { revisions: Object.values(data.theses.theses).reduce((n,r) => n+r.revisions.length,0),
      checkpoints:data.outcomes.checkpoints.length,
      scenarios:Object.values(data.scenarios.calculators).reduce((n,r) => n+r.length,0) };
  }
  function exportJSON() {
    return JSON.stringify({application:'NTM',schemaVersion:1,exportedAt:new Date().toISOString(),data:capture().data},null,2);
  }
  function union(a,b) {
  ...
  function prepare(text) {
    ...
    if (!object(backup) || backup.application !== 'NTM' || backup.schemaVersion !== 1
  ...
  document.getElementById('localDataImport').onclick = () => act(async () => {
    ...
    if (!confirm(`Slå samman ${summary.revisions} versioner, ${summary.checkpoints} utfallskontroller och ${summary.scenarios} scenarier? Nya versioner läggs sist och kan bli senaste version. Samma ID med olika innehåll stoppar hela importen. Befintligt tema behålls.`)) return;
