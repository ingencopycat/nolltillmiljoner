...
function app(saved=new Map()) {
  ...
  c.window=c;
  for(const file of ['valuation-core.js','script.js','research-snapshot.js','thesis-storage.js','research-outcomes.js','behavioral.js','academy-progress.js','local-data.js','cloud-sync.js','cloud-adapter.js'])vm.runInContext(fs.readFileSync(file,'utf8'),c);
