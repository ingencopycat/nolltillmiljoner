...
(function(root){
  'use strict';
  const key='ntm-academy-progress-v1',empty=()=>({version:1,events:[]});
  ...
  function validate(data){
    if(!data || data.version!==1 || Object.keys(data).sort().join()!=='events,version' || !Array.isArray(data.events)||data.events.length>50000)fail();
    const ids=new Set();
  ...
  root.NTMAcademyProgress={key,empty,validate,read,state,summarize,set};
