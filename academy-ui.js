...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(current){
    ...
    document.querySelector('[data-academy-complete]')?.addEventListener('click',()=>{try{const r=P.read();if(r.error)throw new Error(r.error);const before=P.state(r.data),status=before.get(current)?.status==='complete'?'ongoing':'complete';P.set(current,status);const after=P.state(P.read().data);if(status==='complete'){emit('academy_lesson_completed');if(A.paths.some(p=>!A.journey(p.id,before).complete&&A.journey(p.id,after).complete))emit('academy_path_completed');}render();const next=nextLesson(after);if(message)message.textContent=status==='complete'?'Klar och sparad lokalt. '+(next?'Nästa: '+A.lessons.find(l=>l.id===next).title+'.':'Välj ett nytt spår eller pröva ett NTM-verktyg.'):'Lektionen är markerad som pågår. Du kan fortsätta när du vill.';}catch(e){error(e);}});
  }
  ...
  document.querySelectorAll('.academy-quiz').forEach(form=>form.addEventListener('submit',event=>{event.preventDefault();const quiz=A.lessons.find(l=>l.id===current)?.quiz[Number(form.dataset.quiz)];const answer=new FormData(form).get('answer');const output=form.querySelector('[role=status]');output.textContent=answer===null?'Välj ett alternativ för att se förklaringen.':(Number(answer)===quiz.answer?'Så är det. ':'Tänk på underlaget. ')+quiz.explanation;}));
