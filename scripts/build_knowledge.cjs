...
function build(input=raw,options={}){
 ...
 for(const c of categories){const answers=K.publicEntries().filter(e=>e.category===c.id);for(let offset=0;offset<answers.length;offset+=12){const number=offset/12+1,name=`fragor-svar-omrade-${c.id}-${number}.html`;pages.set(name,page(c.title+' – frågor och svar','Granskade frågor inom '+c.title.toLowerCase()+'.',`<h1>${esc(c.title)}</h1><p><a href="fragor-svar.html">Fråga NTM</a></p><ul>${answers.slice(offset,offset+12).map(e=>`<li><a href="${K.url(e.id)}">${esc(e.question)}</a></li>`).join('')}</ul>${offset+12<answers.length?`<a href="fragor-svar-omrade-${c.id}-${number+1}.html">Nästa sida</a>`:''}`));}}
