...
try:
 ...
 for ticker in ('NVDA','SOFI','CRWD'):
  case.go('research.html?ticker='+ticker);p.locator('#thesis-text').fill('Ownership evidence review: preserve saved Research.');p.locator('#thesisForm [type=submit]').click();case.wait_for('t=>NTMThesisStorage.get(t).thesis?.revisionCount>=1',arg=ticker);saved=p.evaluate('t=>NTMThesisStorage.get(t).thesis.revisions',ticker)
  for width in (1440,360,390,430):
   for theme in ('dark','light'):
    ...
    section=p.locator('#companyOwnership');expect(section).to_be_visible();key=f'{ticker}-{width}-{theme}'
 ...
 case.go('research.html?ticker=NVDA');p.evaluate("async()=>{const d=await(await fetch('data/stocks/evidence/NVDA.json')).json();document.querySelector('#companyOwnership').remove();d.ownershipEvidence.pendingReview=[{filingDate:'2026-09-23'}];NTMCompanyOwnershipUI.render(d,document.querySelector('#companyEvidence'),{status:'unavailable'});}")
 ...
 p.evaluate("async()=>{const d=await(await fetch('data/stocks/evidence/NVDA.json')).json();document.querySelector('#companyOwnership').remove();d.ownershipEvidence.filings=[];d.ownershipEvidence.pendingReview=[{filingDate:'2026-09-23'}];NTMCompanyOwnershipUI.render(d,document.querySelector('#companyEvidence'),{status:'verified'});}")
