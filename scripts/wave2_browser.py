...
class Wave2(unittest.TestCase):
    ...
    def save(self,question=False):
        p=self.page
        p.locator('#thesis-text').fill('Private Wave 2 belief')
    ...
    def test_actual_return_all_decisions_and_history(self):
        ...
        for decision in ['keep','revise','close','abstain']:
            with self.subTest(decision=decision):
                ...
                expect(p.locator('#changeDetectionSection')).to_be_visible()
                ...
                if decision=='revise':
                    p.locator('#thesis-text').fill('Revised belief after review')
                    p.locator('#thesisForm [type=submit]').click()
    ...
    def test_draft_restore_discard_no_revision_or_network_leak(self):
        ...
        p.locator('#thesis-text').fill('PRIVATE DRAFT SENTINEL')
    ...
    def test_stale_target_and_concurrent_editor_fail_closed(self):
        ...
        self.page.locator('#review-keep').click()
        ...
        self.page.locator('#thesis-text').fill('Must not overwrite')
        self.page.locator('#thesisForm [type=submit]').click()
    ...
    def test_expired_draft_and_corrupt_companion_preserve_original(self):
        ...
        p.locator('#thesis-text').fill('Expired private draft')
        ...
        p.locator('#draftDeleteCorrupt').click()
    ...
    def test_revise_draft_reload_and_deliberate_reopen(self):
        ...
        p.locator('#thesis-text').fill('Explicit reopened draft')
        p.reload();expect(p.locator('#draftRestore')).to_be_visible();p.locator('#draftRestore').click()
        p.locator('#thesisForm [type=submit]').click()
    ...
    def test_unavailable_stock_and_corrupt_journal_are_not_empty_success(self):
        ...
        self.research();p.locator('#thesis-text').fill('No overwrite')
        p.locator('#thesisForm [type=submit]').click()
    ...
    def test_exact_target_survives_evidence_navigation_reload_and_deletion(self):
        ...
        p.locator('#thesis-text').fill('Must not replace missing target')
        p.locator('#thesisForm [type=submit]').click()
        self.assertIsNone(p.evaluate("NTMThesisStorage.get('NVDA').thesis"))
