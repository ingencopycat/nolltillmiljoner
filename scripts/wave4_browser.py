            ...
            for width in [1440,360,390,430]:
                for theme in ['light','dark']:
                    with self.subTest(ticker=ticker,width=width,theme=theme):
                        ...
                        expect(profile.locator('article:visible')).to_have_count(2)
                        profile.locator('> details > summary').first.focus();p.keyboard.press('Enter')
                        expect(profile.locator('[data-dimension=perShare]')).to_contain_text('Otillräckligt underlag')
                        if ticker=='SOFI':expect(profile.locator('[data-dimension=cash]')).to_contain_text('undanhållet')
                        growth=profile.locator('[data-dimension=growth]');growth.locator('> details > summary').click()
                        expect(growth).to_contain_text('ntm-fundamental/1')
                        if ticker in ['NVDA','SOFI','CRWD']:
                            growth.get_by_role('button',name='Visa källuppgift').first.click()
                        ...
                        profile.locator('> details > summary').first.click();growth.locator('> details > summary').click()
                        profile.locator('> details > summary').last.click()
