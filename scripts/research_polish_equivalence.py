"""Exact pre-Wave-4 vs current outputs and revision payloads, with fixed clock/UUIDs."""
...
BASE='6aec46b5aeea9001167b939cb8e781e6c65e7f6d'
baseline={name:subprocess.check_output(['git','show',BASE+':'+name],cwd=ROOT) for name in ['research.html','research.js','research-shell.js','research-entry.js','research-workspaces.js']}
...
def run(old,ticker):
 ...
 try:
  ...
  outputs['financials']=p.evaluate('({ttm:currentStockData.ttm,annual:currentStockData.annual,quarterly:currentStockData.quarterly,valuationBase:currentStockData.valuationBase})')
...
try:
 ...
 out=ROOT/'docs/qa/research-polish';out.mkdir(parents=True,exist_ok=True)
 evidence={}
 for ticker in ['NVDA','SOFI','CRWD']:
  for name in ['data/stocks/'+ticker+'.json','data/stocks/evidence/'+ticker+'.json']:
   ...
   evidence[name]=hashlib.sha256(json.dumps(json.loads(after),sort_keys=True).encode()).hexdigest()
 (out/'equivalence.json').write_text(json.dumps({'baseline':BASE,'results':results,'unchangedFinancialAndEvidenceJSON':evidence},indent=2),encoding='utf-8')
