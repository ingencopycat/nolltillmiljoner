...
try:
 ...
 for ticker in ('NVDA','SOFI','CRWD'):
  case.go('research.html?ticker='+ticker);p.locator('#thesis-text').fill('Reviewed observations: saved revision remains unchanged.');p.locator('#thesisForm [type=submit]').click();case.wait_for('t=>NTMThesisStorage.get(t).thesis?.revisionCount>=1',arg=ticker);saved=p.evaluate('t=>NTMThesisStorage.get(t).thesis.revisions',ticker)
  for width in (1440,360,390,430):
   for theme in ('dark','light'):
    ...
    section=p.locator('#companyObservations');expect(section).to_be_visible();expect(p.locator('#observationSources')).not_to_have_attribute('open','')
    expect(section).to_contain_text('Bolagets guidning');assert p.locator('#observationSources a:visible').count()==0
    assert p.locator('#companyObservations .observation-chart').count()=={'NVDA':0,'SOFI':1,'CRWD':2}[ticker]
    ...
    section.locator('h2').nth(1).evaluate('(e)=>e.scrollIntoView({block:"start"})');p.screenshot(path=str(OUT/f'{key}-kpis.png'),animations='disabled')
 ...
 case.go('research.html?ticker=SOFI');p.evaluate("async()=>{const d=await (await fetch('data/stocks/evidence/SOFI.json')).json();document.querySelector('#companyObservations').remove();NTMCompanyObservationsUI.render(d,document.querySelector('#companyEvidence'),{status:'unavailable'});}")
