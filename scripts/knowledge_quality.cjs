...
function validate(data,registry){
 ...
 for(const e of data.entries){
  ...
  if(Object.keys(e).some(k=>![...Core.fields,'internalEditorialNotes'].includes(k))||!['draft','reviewed','published','needs-update'].includes(e.status)||!categories.has(e.category)||!['beginner','intermediate','advanced'].includes(e.difficulty))errors.push('Invalid editorial metadata '+e.id);
