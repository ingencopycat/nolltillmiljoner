...
class Knowledge100(unittest.TestCase):
    ...
    def ask(self, query):
        self.page.locator('#question').fill(query)
    ...
    def test_mobile_keyboard_clarification_formula_and_depth(self):
        ...
        for width in [360, 390, 430]:
            ...
            for theme in ['light', 'dark']:
                ...
                self.ask('avkastning')
                expect(p.locator('#answer .choice')).to_have_count(3)
                p.locator('#answer .choice').first.focus()
                p.keyboard.press('Enter')
                expect(p.locator('#answer')).to_contain_text('Vad skiljer CAGR')
                p.locator('#answer .depth summary').press('Enter')
                expect(p.locator('#answer .depth')).to_have_attribute('open','')
                self.ask('Vad är P/E?')
                p.locator('.formula summary').press('Enter')
                p.locator('.formula-form [name=price]').fill('150')
                p.locator('.formula-form [name=eps]').fill('6')
                p.locator('.formula-form button').click()
                expect(p.locator('#answer')).to_contain_text('25 gånger')
                p.locator('.formula summary').press('Enter')
                p.locator('.formula-form [name=price]').fill('150')
                p.locator('.formula-form [name=eps]').fill('-6')
                p.locator('.formula-form button').click()
                expect(p.locator('#answer')).to_contain_text('Negativ EPS')
                self.ask('Är P/E 40 dyrt?')
                expect(p.locator('#answer')).to_have_attribute('data-outcome','ANSWER')
                expect(p.locator('#answer')).to_contain_text('avgör inte ensamt')
                ...
                expect(p.locator('#answer')).to_contain_text('saknas ett granskat svar')
                p.locator('[data-mode=search]').click()
                p.locator('#searchInput').fill('EPS')
    ...
    def test_body_failure_and_no_initial_corpus_download(self):
        ...
        self.ask('Vad är P/E?')
        expect(p.locator('#answer')).to_contain_text('Svaret kunde inte laddas säkert')
    ...
    def test_synthetic_scale_browser(self):
        ...
        for size in [100,500,1200]:
            ...
            p.locator('[data-mode=search]').click()
            p.locator('#searchInput').fill('vinst')
            p.locator('#category').select_option('metrics')
            self.assertLessEqual(p.locator('[data-knowledge-card]:visible').count(),12)
