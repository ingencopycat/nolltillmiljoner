...
class BrowserSmoke(unittest.TestCase):
    ...
    def test_company_evidence_pilot(self):
        for ticker in ('NVDA', 'SOFI', 'CRWD'):
            ...
            self.open_research_workspace('#companyEvidence')
            expect(panel).to_be_visible()
            expect(panel).to_contain_text('Läs resultatmeddelande')
            expect(panel.locator('#reportingSources')).not_to_have_attribute('open', '')
            expect(panel.locator('#reportingSources')).to_contain_text('Nytt underlag är ingen investeringsslutsats')
            self.assertTrue(panel.get_by_role('link', name='Läs resultatmeddelande').first.get_attribute('href').startswith('https://www.sec.gov/Archives/'))
            self.page.evaluate("NTMCompanyEvidence.setBaseline('2026-01-01T00:00:00Z')")
            expect(self.page.locator('#companyEvidenceSince')).to_be_hidden()
            expect(self.page.locator('#researchSince')).to_contain_text('Spara en analys')
    ...
    def open_depth_for(self, selector):
        ...
        self.open_research_workspace(selector)
    ...
    def test_wave0_credibility_in_real_pages(self):
        ...
        for query, answer in [('Vad är P/E?', 'Vad betyder P/E?'), ('Vad är forward P/E?', 'Vad skiljer forward P/E från trailing P/E?')]:
            p.locator('#question').fill(query)
            p.locator('#question').press('Enter')
            expect(p.locator('#answerTitle')).to_have_text(answer)
    ...
    def test_knowledge_bank_search_links_and_accessibility(self):
        ...
        p.locator('#question').fill('Vad är PEG?')
        p.locator('#question').press('Enter')
        expect(p.locator('#answerTitle')).to_have_text('Vad betyder PEG?')
        p.locator('#question').fill('PRIVATE-UNKNOWN-QUERY')
        p.locator('#question').press('Enter')
        ...
        p.locator('#searchInput').fill('vinst per aktie')
        ...
        for query, answer in [('snittavkastning','cagr'),('mäklaravgift','fees'),('P/E','pe')]:
            p.locator('#searchInput').fill(query)
            expect(p.locator(f'[data-knowledge-card={answer}]')).to_be_visible()
        p.locator('#searchInput').fill('PRIVATE-UNKNOWN-QUERY')
        expect(p.locator('#results > a')).to_have_count(0)
        self.assertNotIn('PRIVATE',p.evaluate('JSON.stringify(NTMEvents.snapshot())'))
        self.assertNotIn('PRIVATE',p.url)
        p.locator('[data-mode=explore]').click()
        p.locator('#categories button').filter(has_text='Värdering').click()
        expect(p.locator('#concepts details').first).to_be_visible()
        self.go('fragor-svar-pe-tal.html')
        ...
        expect(popup.value.locator('h1')).to_contain_text('P/E')
        popup.value.close()
        with p.expect_popup() as popup:
            p.locator('.knowledge-related a[href="aktievarderingskalkylator.html"]').click()
        self.assertIn('aktievarderingskalkylator.html',popup.value.url)
        popup.value.close()
        ...
        try:
            ...
            expect(page.locator('[data-knowledge-card]')).to_have_count(0)
            expect(page.locator('#knowledgeExperience')).to_be_hidden()
            page.locator('a[href="fragor-svar-omrade-metrics-1.html"]').click()
            page.get_by_role('link', name='Vad är EPS?', exact=True).click()
    ...
    def test_research_expansion_batches_one_and_two(self):
        ...
        for ticker in ['MU', 'MRVL', 'VRT', 'COHR', 'RKLB', 'TTMI', 'SNDK', 'FLY', 'CRWV']:
            ...
            self.open_research_workspace('#val-price')
            p.locator('#val-price').fill('123')
            self.open_research_workspace('#val-eps')
            p.locator('#val-eps').fill('2')
            self.open_research_workspace('#valuationForm button[type=submit]')
            ...
            self.open_research_workspace('#thesis-text')
            p.locator('#thesis-text').fill('Synthetic browser thesis ' + ticker)
            self.open_research_workspace('#thesisForm button[type=submit]')
            ...
            self.open_research_workspace('#outcomeForm button[type=submit]')
    ...
    def test_behavioral_intelligence_local_workflows(self):
        ...
        for index in [0, 1]:
            self.open_research_workspace('#assumptionSelect')
    ...
    def test_research_ai_foundation(self):
        ...
        self.open_research_workspace('#thesis-assumption-1')
        p.locator('#thesis-assumption-1').fill('Revenue remains strong')
        self.open_research_workspace('#thesisForm button[type=submit]')
        p.locator('#thesisForm button[type=submit]').click()
        self.open_depth_for('#researchAI')
        expect(p.locator('#researchAI')).to_be_visible()
        self.open_research_workspace('#researchAI > details > summary')
        self.open_depth_for('#researchAI')
    ...
    def test_growth_reminders_return_private_and_mobile(self):
        ...
        self.open_research_workspace('#thesis-review-date')
        ...
        self.open_research_workspace('#assumption-date-1')
        ...
        self.open_research_workspace('#report-question-1')
        ...
        self.open_research_workspace('#draftDiscard')
        p.locator('#draftDiscard').click()
        self.open_depth_for('#reportQuestionsEditor > summary')
        p.locator('#reportQuestionsEditor > summary').click()
        self.open_research_workspace('#report-status-1')
        ...
        self.open_research_workspace('#review-close')
    ...
    def test_trust_content_journeys_and_instagram(self):
        ...
        self.open_research_workspace('#reviewQueue a')
        p.locator('#reviewQueue a').click()
        self.open_depth_for('#review-keep')
        self.open_research_workspace('#review-keep')
    ...
    def test_connected_journeys_and_private_editor_preservation(self):
        ...
        self.wait_for("typeof currentStockData !== 'undefined' && currentStockData?.symbol==='NVDA'")
        self.open_depth_for('.ntm-relations:not([hidden])')
        ...
        self.go('research.html?ticker=NVDA')
        self.open_depth_for('[data-relation-id=nvda-content]')
        ...
        self.open_depth_for('[data-relation-id=nvda-thesis]')
    ...
    def test_premium_depth_keyboard_tables_and_reduced_motion(self):
        ...
        self.open_research_workspace('.metric-info-btn')
    ...
    def test_color_v2_controls_semantics_charts_and_theme_persistence(self):
        ...
        for theme in ['dark', 'light']:
            ...
            self.open_depth_for('#annualChart')
    ...
    def test_shared_valuation_surfaces_and_sensitivity(self):
        ...
        for field,value in {'val-price':'80','val-eps':'4','val-return':'25','val-years':'2','val-exit-pe':'20',
            ...
            self.open_research_workspace('#'+field)
            p.locator('#'+field).fill(value)
        self.open_research_workspace('#valuationForm button[type="submit"]')
        p.locator('#valuationForm button[type="submit"]').click()
    ...
    def test_decision_lifecycle_supported_and_manual_backup(self):
        ...
        self.open_depth_for('.assumption-detail > summary')
        p.locator('.assumption-detail > summary').first.click()
        self.open_research_workspace('#assumption-falsification-1')
        ...
        self.open_research_workspace('#thesisRevisionSelect')
        ...
        self.open_research_workspace('#review-abstain')
        ...
        self.open_research_workspace('#researchExportPrint')
        ...
        self.open_research_workspace('#thesisDeleteBtn')
    ...
    def test_research_history_export_outcomes_and_backup_roundtrip(self):
        ...
        self.open_research_workspace('#revisionRestoreBtn')
        ...
        self.open_research_workspace('#outcomeManualPrice')
    ...
    def test_product_paths_mobile_navigation_and_second_review(self):
        ...
        self.assertEqual(p.locator('.main-nav > a').all_text_contents(),['Verktyg','Research','Fråga NTM','Academy','Min NTM','Logga in'])
        p.set_viewport_size({'width':390,'height':844})
        p.locator('#mobileNavToggle').click()
        ...
        self.open_research_workspace('#reviewNextDate')
    ...
    def test_weekly_macro_today_tomorrow_and_expansion(self):
        ...
        expect(p.locator('#ntmMacroList')).not_to_contain_text('delvis uppdaterade')
        ...
        expect(p.locator('#ntmMacroList')).not_to_contain_text('delvis uppdaterade')
        self.assertEqual(p.evaluate('NTM_WEEKLY_EVENTS.meta.status'), 'partial')
