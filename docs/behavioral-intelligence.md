...
Academy V1 update: portable exports now use backup V3 with an additional `academy` history. V1/V2 imports remain supported. Behavioral records keep their original event model and merge protections. Both behavioral and Academy histories remain excluded from the current cloud adapter. See [Academy architecture](academy.md).
