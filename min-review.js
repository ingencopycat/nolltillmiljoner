...
(() => {
  ...
  function attention(thesis, data, today) {
    ...
    if (state === 'closed' || state === 'missing') return [];
    const reasons = state === 'due' ? ['Ditt granskningsdatum har nåtts'] : [];
    if (data && thesis.valuationSnapshot) {
  ...
  async function render() {
    ...
    const rows=await Promise.all(Object.values(store.theses).map(async thesis => {
      if (thesis.review?.decision === 'close') return {thesis,reasons:[],closed:true};
      let data=null, unavailable=false;
      if (['NVDA','SOFI','CRWD'].includes(thesis.ticker)) {
    ...
    for(const row of rows) {
      ...
      title.textContent=`${row.thesis.ticker} · ${row.thesis.companyName || ''}`;
      const savedDate=Date.parse(row.thesis.updatedAt);
      detail.textContent=`Senaste version: ${Number.isFinite(savedDate) ? new Date(savedDate).toLocaleDateString('sv-SE') : 'datum saknas'}. ${row.closed ? 'Stängd tes; historiken finns kvar.' : row.reasons.join('. ')+'.'}`;
      link.href=`research.html?ticker=${encodeURIComponent(row.thesis.ticker)}&review=1#thesisReview`;
      link.className='secondary-btn';link.textContent=row.closed?'Läs stängd tes':'Granska tes';
